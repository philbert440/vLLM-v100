...
try:
    ...
    Eagle3DeepseekV2ForCausalLM = None
...
def _dflash_dump_draft_logits_enabled() -> bool:
    return os.getenv("VLLM_DFLASH_DUMP_DRAFT_LOGITS", "0") == "1"


def _spec_debug_corruption_enabled(method: str) -> bool:
    if method == "dflash" and _dflash_debug_corruption_enabled():
        return True
    return os.getenv("VLLM_SPEC_DEBUG_CORRUPTION", "0") == "1"


def _spec_dump_draft_logits_enabled(method: str) -> bool:
    if method == "dflash" and _dflash_dump_draft_logits_enabled():
        return True
    return os.getenv("VLLM_SPEC_DUMP_DRAFT_LOGITS", "0") == "1"


def _dump_dflash_debug(payload: dict[str, Any], suffix: str) -> str:
    dump_path = f"/tmp/dflash_{suffix}_pid{os.getpid()}.pt"
...
def _dump_spec_debug(payload: dict[str, Any], method: str, suffix: str) -> str:
    if method == "dflash":
        return _dump_dflash_debug(payload, suffix)
    dump_path = f"/tmp/spec_{method}_{suffix}_pid{os.getpid()}.pt"
    torch.save(payload, dump_path)
    return dump_path
...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        # When True, all draft steps reuse the same position as the first step
        # instead of advancing by one each iteration. Used by draft models with
        # Q-only attention that share KV with the target and always predict from
        # the same position (e.g. Gemma4 MTP).
    ...
    def propose(
        ...
        if self.method in ("eagle3", "dflash"):
            supported_models = [Eagle3LlamaForCausalLM, DFlashQwen3ForCausalLM]
            if Eagle3DeepseekV2ForCausalLM is not None:
                supported_models.append(Eagle3DeepseekV2ForCausalLM)
            assert isinstance(
                self.model,
        ...
        if should_collect_draft_logits:
            ...
            debug_summary = {
                ...
                "sample_hidden_state_nan_counts": (
                    sample_hidden_states.isnan().sum(dim=-1).detach().cpu()
                ),
                "sample_hidden_state_norms": (
                    sample_hidden_states.float().norm(dim=-1).detach().cpu()
                ),
        ...
        if self.constant_draft_positions:
            # Write the sampling positions into the front of the positions
            # buffer so subsequent loop iterations (which read via
            # _get_positions) use the correct values.
        ...
        for token_index in range(self.num_speculative_tokens - 1):
            ...
            # (e.g. Gemma4 MTP), common_attn_metadata is invariant across loop
            # iterations so we build once (token_index == 0) and reuse.
    ...
    def _update_positions_dependent_metadata(
        ...
        """Update positions, slot mappings, and sequence metadata for the next
        draft step. Returns the updated positions tensor. Extracted from
        propose() so the Gemma4-MTP constant-positions path can skip it."""
        # Use fused kernel for slot mapping and metadata updates. Write clamped
        # positions directly into the positions buffer to avoid an extra D2D
        # copy for the common (non-mrope) case.
        ...
        # Increment max_seq_len unconditionally (upper bound for seq lengths).
        common_attn_metadata.max_seq_len = min(
            common_attn_metadata.max_seq_len + 1, self.max_model_len
        )
        # Update the CPU-side shadow.
    ...
    def initialize_attn_backend(
        ...
        for layer_name in self._draft_attn_layer_names:
            ...
            group_key = (kv_cache_gid, backend_key)
        ...
        if self.vllm_config.parallel_config.data_parallel_size > 1:
            ...
            if num_tokens_across_dp is not None:
                ...
                if use_cudagraphs:
                    ...
                    cudagraph_mode = CUDAGraphMode.NONE
                    batch_desc = BatchDescriptor(num_tokens_padded)
