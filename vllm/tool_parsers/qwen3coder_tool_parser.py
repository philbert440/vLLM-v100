...
class Qwen3CoderToolParser(ToolParser):
    ...
    def _convert_param_value(
        ...
        if (
            ...
            # anyOf has no top-level type; parse as JSON when possible.
            param_type = "object"
        else:
            param_type = "string"
    ...
    def _parse_xml_function_call(
        ...
        param_config = self._get_arguments_config(function_name, tools or self.tools)
    ...
    def extract_tool_calls(
        ...
        try:
            ...
            valid_tool_calls = [tool_call for tool_call in tool_calls if tool_call]
    ...
    def extract_tool_calls_streaming(
        ...
        if not self.header_sent:
            if self.tool_call_prefix in tool_text:
                ...
                if func_end != -1:
                    ...
                    # Each tool call is a separate invocation even if the same
                    # function name appears multiple times.
                    self.prev_tool_call_arr.append(
                        {
        ...
        if self.in_function:
            # Always send opening brace first. A single speculative delta can
            # contain both the first parameter and its value.
            ...
            # Process every complete parameter available in this delta. With
            # speculative decoding, a burst can contain multiple parameters.
            json_fragments = []
            ...
            # Check for function end after parameters so the final parameter is
            # not dropped when it arrives in the same delta as </function>.
